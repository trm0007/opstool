...
# Add this temporarily to test3.py to see what's there
import os
print("Files in /app:")
...
def create_slab(slab_configs, output_dir=None):
    ...
    for config in slab_configs:
        config['py_file'] = os.path.join(output_dir, config['py_file'])
        config['png_file'] = os.path.join(output_dir, config['png_file'])
...
def create_dynamic_composite_section(
    ...
    for file_path in [save_txt_path, save_png_path, save_pkl_path]:
        if file_path:
...
def create_fiber_section(fiber_configs, output_dir=None):
    """
    ...
    ----------
    fiber_configs : dict
        Dictionary containing all section parameters.
    output_dir : str, optional
        Output directory for files. If None, uses current directory.
    ...
    # If output_dir is provided, prepend it to save_prefix
    if output_dir:
        save_prefix = os.path.join(output_dir, save_prefix)
...
def build_model(model_params, fiber_configs, material_params,
    ...
    if slab_configs:
        ...
        slab_creation_results = create_slab(slab_configs, output_dir=output_dir)
